...
<html>
    <head>
        ...
        <title>Listado de Dueños de Locales</title>
        ...
        <style>
            ...
            /* Estilo para el mensaje de "no hay dueños" */
            .no-owners-message {
                text-align: center;
                padding: 25px;
    ...
    <body>
        <div class="container">
            <h1>Listado de Dueños de Locales</h1>

            <%
                // --- INICIO CÓDIGO DEPURACIÓN CRÍTICA (NO ELIMINAR) ---
                System.out.println("*************** listadoOwners.jsp HA EMPEZADO A EJECUTARSE ***************");
                // --- FIN CÓDIGO DEPURACIÓN CRÍTICA ---

                System.out.println("listadoOwners.jsp: Intentando obtener 'datosOwners' de la sesión.");

                // Recuperar la matriz de datos de la sesión
                String datosOwners[][] = (String[][]) session.getAttribute("datosOwners");
                
                if (datosOwners == null || datosOwners.length == 0) {
                    // --- INICIO CÓDIGO DEPURACIÓN (NO ELIMINAR) ---
                    if (datosOwners == null) {
                        System.out.println("listadoOwners.jsp: 'datosOwners' es NULL en la sesión.");
                    } else if (datosOwners.length == 0) {
                        System.out.println("listadoOwners.jsp: 'datosOwners' es una matriz vacía (length = 0).");
                    }
                    // --- FIN CÓDIGO DEPURACIÓN ---
                    out.print("<p class='no-owners-message'>No hay dueños de locales para mostrar o no se pudieron cargar los datos.</p>");
                } else {
                    // --- INICIO CÓDIGO DEPURACIÓN (NO ELIMINAR) ---
                    System.out.println("listadoOwners.jsp: Se encontraron " + datosOwners.length + " filas de dueños.");
                    // --- FIN CÓDIGO DEPURACIÓN ---
                    ...
                    // Para 'owner_local': cedulaOwner, nombreOwner, apellidoOwner, correoOwner, passwordOwner, telefonoOwner, estado
                    out.print("<th>Cédula</th>");      // registro[0]
                    out.print("<th>Nombre</th>");      // registro[1]
                    out.print("<th>Apellido</th>");    // registro[2]
                    out.print("<th>Correo</th>");      // registro[3]
                    // out.print("<th>Contraseña (NO MOSTRAR)</th>"); // registro[4] - No se muestra por seguridad
                    out.print("<th>Teléfono</th>");    // registro[5]
                    out.print("<th>Estado</th>");      // registro[6]
                    out.print("</tr></thead>");

                    out.print("<tbody>");
                    // Iterar sobre cada fila (dueño)
                    for (String[] registro : datosOwners) {
                        out.print("<tr>");
                        // Asegúrate de que los índices coincidan con la consulta en Conexion.java
                        out.print("<td>" + registro[0] + "</td>"); // cedulaOwner
                        out.print("<td>" + registro[1] + "</td>"); // nombreOwner
                        out.print("<td>" + registro[2] + "</td>"); // apellidoOwner
                        out.print("<td>" + registro[3] + "</td>"); // correoOwner
                        // out.print("<td>" + registro[4] + "</td>"); // passwordOwner - NO IMPRIMIR POR SEGURIDAD
                        out.print("<td>" + registro[5] + "</td>"); // telefonoOwner
            ...
            %>
            <a href="menuAdmin.jsp" class="back-link">Volver al menú</a> 
        </div>
    </body>
</html>